...
export default async function AdminDocumentsPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f2f7fb] text-[#40566a]">
    <header className="border-b border-[#cbdce8] bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4"><Link href="/admin" className="font-bold text-[#17375e]">← Painel administrativo</Link><Link href="/documentos" className="text-sm font-semibold text-[#315f7d]">Ver biblioteca pública</Link></div></header>
    <section className="mx-auto max-w-7xl px-6 py-12">
      ...
      <p className="mt-4 max-w-3xl leading-7">Cadastre PDFs oficiais e escolha se serão publicados imediatamente ou mantidos como rascunho.</p>
      ...
      <form method="post" action="/api/admin/documentos" encType="multipart/form-data" className="mt-8 grid gap-6 rounded-2xl border border-[#d5e3ec] bg-white p-6 shadow-[0_8px_24px_rgba(31,88,114,.06)]">
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <Field label="Situação"><select name="status" className="input"><option value="published">Publicado</option><option value="draft">Rascunho</option></select></Field>
          <Field label="Visibilidade"><select name="visibility" className="input"><option value="public">Público</option><option value="restricted">Restrito</option><option value="internal">Interno</option></select></Field>
        </div>
      ...
      <div className="mt-5 overflow-x-auto rounded-2xl border border-[#d5e3ec] bg-white"><table className="w-full min-w-[860px] text-left text-sm">
        <thead className="bg-[#e8f2f8]"><tr><th className="p-4">Documento</th><th className="p-4">Tipo</th><th className="p-4">Ano</th><th className="p-4">Situação</th><th className="p-4">Visibilidade</th><th className="p-4">Ação</th></tr></thead>
        <tbody>{documents.map((document) => <tr key={document.id} className="border-t border-[#e2ebf1]"><td className="p-4 font-bold text-[#234c68]">{document.title}</td><td className="p-4">{document.typeName ?? "Sem tipo"}</td><td className="p-4">{document.referenceYear ?? "—"}</td><td className="p-4">{document.status === "published" ? "Publicado" : document.status === "draft" ? "Rascunho" : "Arquivado"}</td><td className="p-4">{document.visibility === "public" ? "Público" : document.visibility === "restricted" ? "Restrito" : "Interno"}</td><td className="p-4"><Link href={`/admin/documentos/${document.id}`} className="font-bold text-[#087f5b] hover:underline">Revisar</Link></td></tr>)}</tbody>
      </table></div>
    </section>
  </main>;
}
