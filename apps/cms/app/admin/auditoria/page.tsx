...
export default async function AuditPage({ searchParams }: { searchParams: Promise<{ tipo?: string; pagina?: string }> }) {
  ...
  const type = ["document", "conference_proposal"].includes(query.tipo ?? "") ? query.tipo! : null;
  const requestedPage = Number(query.pagina ?? "1");
  const page = Number.isInteger(requestedPage) && requestedPage > 0 ? requestedPage : 1;
  const result = await listAuditEntries(type, page);
  ...
  return <main className="min-h-screen bg-[#f2f7fb] text-[#40566a]">
    ...
    <section className="mx-auto max-w-6xl px-6 py-12">
      ...
      <nav aria-label="Filtrar histórico" className="mt-8 flex flex-wrap gap-3">{[[null, "Todos"], ["document", "Documentos"], ["conference_proposal", "Propostas"]].map(([value, label]) => <Link key={label} href={filterHref(value)} className={`rounded-full px-4 py-2 text-sm font-bold ${type === value ? "bg-[#315f7d] text-white" : "border border-[#9db9ca] bg-white text-[#315f7d]"}`}>{label}</Link>)}</nav>
